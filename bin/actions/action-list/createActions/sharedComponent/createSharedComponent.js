...
export const createSharedComponent = (fsdPath, componentName) => {
    ...
    try {
        ...
        fs.writeFileSync(resolveUI(componentName, `${componentName}.stories.tsx`), storyUITemplate(componentName));
        ...
        if (isExistUICollector) {
            fs.appendFileSync(resolve('shared', 'ui', 'index.ts'), indexComponentTemplate(componentName));
        }
        else {
            fs.writeFileSync(resolve('shared', 'ui', 'index.ts'), indexComponentTemplate(componentName));
        }
...
//# sourceMappingURL=createSharedComponent.js.map